...
`0008_continuous_budget_periods.sql` 将用户和 Key 的日／周金额窗口改为连续周期：日窗口仍按北京时间零点，
...
管理员登录后，在「用户管理」创建账号、设置初始密码、分配账号分组及日／周限额、共享并发和 RPM。
普通用户未分配分组时不能调用模型。管理员与普通用户共用原登录地址；管理员侧栏下方及普通用户侧栏
提供「个人资料」「我的 Key」「使用记录」。「个人资料」统一查看额度、权限及修改密码；普通用户默认进入
`/me/profile`，旧 `/me/overview` 和 `/me/settings` 自动跳转。新建个人 Key 继承用户权限，默认不再附加 Key 限额。

用户停用会使全部旧会话失效，其 Key 拒绝新请求；已开始请求允许完成并结算。修改或重置密码使旧会话
失效，但不自动撤销 Key；需要同时阻止 API 调用时停用用户或相关 Key。至少保留一个启用的管理员。
不开放自助注册，不提供套餐、订阅和支付流程。

用户列表的删除操作通过新增迁移 `0007_user_deletion.sql` 支持。删除后用户及所属 Key 不再可用，已有
