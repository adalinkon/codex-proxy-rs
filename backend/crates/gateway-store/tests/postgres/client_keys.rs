...
async fn migrated_keys_persist_exactly_and_keep_short_keys_masked() {
    ...
    for (index, key) in values.iter().enumerate() {
        ...
        assert_eq!(record.user_id, "test-owner");
...
async fn client_key_search_matches_names_and_labels_but_never_credential_values() {
    ...
    for (id, name, key) in [
        ...
        sqlx::query(
            "insert into client_api_keys (user_id, id, name, label, key, created_at, updated_at)
            values ('test-owner', $1, $2, 'backend label', $3, now(), now())",
        )
        .bind(id)
        .bind(name)
        .bind(key)
    ...
    for (search, expected) in [
        ...
        for user_id in [None, Some("test-owner"), Some("other-user")] {
            let page = repository
                ...
                .await
                .unwrap();
            let expected = if user_id == Some("other-user") {
                0
            } else {
                expected
            };
            assert_eq!(page.total, expected, "search: {search}, user: {user_id:?}");
...
async fn client_key_usage_touch_should_preserve_the_latest_observed_timestamp() {
    ...
    sqlx::query(
        "insert into client_api_keys (user_id,
           id, name, key, enabled, max_concurrency, requests_per_minute,
           created_at, updated_at
         ) values ('test-owner', 'key_usage_touch', 'usage', $1, true, 0, 0, now(), now())",
    )
...
async fn client_key_usage_daemon_should_flush_pending_touch_on_shutdown() {
    ...
    sqlx::query(
        "insert into client_api_keys (user_id,
           id, name, key, enabled, max_concurrency, requests_per_minute,
           created_at, updated_at
         ) values ('test-owner', 'key_usage_daemon', 'usage', $1, true, 0, 0, now(), now())",
    )
...
async fn dedicated_reveal_returns_plaintext_without_debug_exposure() {
    ...
    sqlx::query(
        "insert into client_api_keys (user_id,
           id, name, key, enabled, max_concurrency, requests_per_minute,
           created_at, updated_at
         ) values ('test-owner', 'key_reveal', 'reveal', $1, true, 1, 2, now(), now())",
    )
    .bind(&plaintext)
    .execute(&database.pool)
    .await
