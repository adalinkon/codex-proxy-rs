...
- 管理员与普通用户共用登录入口、普通用户的 `/api/user/*` 个人面板；
...
- Key 没有分组关联时继承用户授权，只有明确拥有全部账号权限的管理员才得到 `AllAccounts`；
- 有关联时与用户授权分组取交集，只允许其中已启用分组成员的并集；
...
### 用户与 Client Key 限额及结算

用户沿用 `admin_users` 身份表扩展两种角色；不开放注册，不引入套餐、订阅、权益快照或新增用户操作审计。
Admin 拥有用户管理与密码用例，API 校验管理员角色、会话及资源归属，Store 保存用户密码摘要、认证版本、
授权分组和直接绑定的额度。Redis 会话保存认证版本；每次会话校验读取当前用户启停和版本，密码修改／重置、
启停及角色改变使旧会话失效。至少保留一个启用的管理员。

用户删除以 `deleted_at` 标记保留身份外键，并在同一配置事务中禁用用户、推进认证版本、删除所属 Key
及用户分组关联。列表与认证只读取未删除用户，配置与密码写入不能恢复已删除身份。用户费用与请求归属
不受删除影响；用户名保留以避免同名重建接管历史记录，最后一个启用的管理员受保护。

Key 必须有不可变的用户归属。Snapshot 从同一一致性视图读取用户启停、授权分组及请求限制，Core 在
账号权限 owner 内编译用户与 Key 的有效范围。普通用户空分组表示无权限，Key 空分组表示继承；不把
两个分组集合交集为空解释为全部账号。用户停用后所属 Key 拒绝新请求。

执行顺序为 Key 鉴权、用户状态、基础校验与最低客户端版本、有效授权与 Provider 候选、金额、两层请求
名额、上游账号选择、执行与结算。金额依次判断用户日／周和 Key 日／周；Redis 用一次 Lua 调用先检查
用户及 Key 的 RPM／并发，再同时写入名额，失败不留下部分扣计。上游账号并发仍在 Provider 选择账号时检查。
请求冻结用户和授权范围，内部重试不重新取用户／Key 名额；长连接的下一轮执行重新读取最新策略。

日金额、七天金额、并发和 RPM 按 Client Key 跨账号、跨 Provider 合计，零表示不限；修改限额不重置已用金额。
Core 负责准入与结算时序，Store 持久化费用账本，Admin 负责限额配置。

同一用户的所有 Key 还共享用户级日／七天金额、并发与 RPM。用户和 Key 费用在同一 PostgreSQL 事务中
结算，锁顺序为用户再 Key；采用不阻塞外键引用的行锁，避免与管理写入产生反向锁依赖。用户费用事件以
请求 ID 幂等保存，保留原 Key 引用但不引用 Key 外键；删除 Key 不删除用户账本，已开始请求仍向原用户结算。
`model_requests.user_id` 保存入口归属，个人用量过滤不通过当前 Key 表反推历史用户。

个人统计路由位于 API 的 `admin/observability/user_routes.rs`，复用 Admin 观测用例及展示映射；其窄查询
合同不接受用户 ID、账号过滤或账号聚合维度，服务端从已验证会话注入归属。个人列表与趋势响应移除账号身份
和全局账号容量，搜索不匹配隐藏账号字段。Vue 的个人 Key、统计路由直接复用原页面，通过固定 API 范围取数；
Key 的名称和标签更新走 Admin 的归属用例，由 Store 在同一事务中校验所有者且仅更新这两个字段。

- 日窗口按北京时间零点划分；七天窗口以用户或 Key 各自创建当天零点为锚点，持续按七天推进，不依赖请求或启停。
...
Store 的 `budget_periods` 统一提供北京时间日／周边界，状态视图和准入／结算调用同一函数；页面查询
只投影当前窗口的累计值，过期窗口按零展示，边界写入按需推进，不扫描账本也不依赖定时清零任务。
准入和结算在已持有用户／Key 行锁后，以同一条 SQL 按需推进窗口并返回金额状态；周期未变时直接读取，
不重写窗口。请求的 `admission_decision_ms` 覆盖金额检查与 Redis 名额准入的总耗时。
Admin 用户用例提供主动重置，API 仅允许管理员并校验幂等标识；Store 在用户行锁内保存实际清零时间、
重建用户窗口并保存操作去重结果。`budget_reset_at` 同时作为清零截止和新的周锚点，旧完成时间的补写费用
不进入新用户计数，Key 仍按自身周期累计。重置不修改策略或认证版本，不需要发布运行快照，也不新增审计事件。

实时用户／Key 请求计数由 Admin 用户用例通过 `RequestUsageStore` 查询。Store adapter 先用 PostgreSQL
验证资源存在与 Key 归属，再批量读取准入实际使用的 Redis ZSET；使用 Redis 时钟计算有效并发及滚动
60 秒 RPM，只读计数，不修改 TTL 或准入状态。Redis 查询有一秒时限，失败返回未知计数，不能伪装成零。
Vue 对当前可见页单独轮询计数，避免重新加载策略列表；页面隐藏、切换或卸载时取消查询，过期响应不回填。
...
结算写入失败时，进程内保留精确费用，在同一用户任意 Key 下次请求前重试；进程退出后无法恢复的费用不会形成欠账。
...
| 用户授权、认证版本、金额窗口与费用事件 | PostgreSQL | 用户身份、共享限制及独立于 Key 生命周期的权威账本 |
